const GAME_URL = 'json/video_game_sales_data.json';
const MOVIE_URL = 'json/movie_data.json';
const KICK_URL = 'json/kickstarter_funding_data.json';
...
async function start() {
  try {
    const [movieDataSet, gameDataSet, kickDataSet] = await Promise.all([
      fetchAndParseJson(MOVIE_URL),
      fetchAndParseJson(GAME_URL),
      fetchAndParseJson(KICK_URL),
    ]);
...
function buildSVG(svgLocation, dataSet, testSet) {
  ...
  var colorScale = d3.scaleOrdinal().domain(colorDomain).range(colorRange);
  ...
  for (var k = 0; k < colorDomain.length; ++k) {
    legend
      ...
      .attr(
        'y',
        (d) => fullheight - padding - 5 - 1.5 * legendSize * currentLabelY
      )
      .text(colorDomain[k]);
    if ((k + 1) % 5 == 0) {
      ++currentLabelY;
